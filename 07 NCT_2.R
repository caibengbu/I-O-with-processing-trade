...
decom$region_name <- ifelse(decom$region_name == "Inner Mongolia", "InnerMong",decom$region_name)
...
for(i in 1:length(prov))
{
  ...
  #assign("self_tmp",eval(parse(text = self.name[i])))
  #IMcof_tmp[,i] <- self_tmp
...
Zp = apply(mat_4d,c(1,3,4),sum)
containerZ = c()
for(i in 1:length(prov))
{
  assign("NCT_tmp",eval(parse(text = NCT.name[i])))
  containerZ = cbind(containerZ,NCT_tmp)
}
containerZ = array(containerZ,dim = c(42,47,31))
ZZ = containerZ-Zp
a = apply(ZZ[,1:46,],c(1,3),sum)
a = a<0
for(i in 1:length(prov))
{
  c = a[,i]
...
allprov_EX = read_csv('allprov_EX.csv')
allprov_ERR = read_csv('allprov_ERR.csv')
tail = cbind(allprov_EX$x,allprov_ERR$x)
concatenate <- cbind(as.matrix(concatenate),tail)
fill.na1 <- matrix(0,nrow = 4, ncol = dim(concatenate)[2]-dim(V)[2])
fill.na2 <- matrix(0,nrow = ns, ncol = 2)
IM <- cbind(IMz,IMf,fill.na2)
concatenate <- rbind(as.matrix(concatenate),as.matrix(IM))
third_quadrant <- cbind(V,fill.na1)
...
write.csv(concatenate,file = "IOmatrix_int1.csv")
